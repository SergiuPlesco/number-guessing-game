...
MAIIN_FUNCTION() {
  ...
  if [[ -z $PLAYER_ID ]]; then
    ...
    INSERT_NEW_PLAYER=$($PSQL "INSERT INTO players(username) VALUES('$NAME');")

   
    PLAYER_ID=$($PSQL "SELECT player_id FROM players WHERE username = '$NAME';")
  else
    USERNAME=$($PSQL "SELECT username FROM players WHERE username = '$NAME';")
    GAME=$($PSQL "SELECT p.player_id, p.username, COUNT(g.game_id) AS GAMES_PLAYED, MIN(g.guess_count) AS best_game FROM players p LEFT JOIN games g USING(player_id) WHERE g.player_id = $PLAYER_ID GROUP BY p.player_id, p.username")
    IFS="|" read PLAYER_ID USERNAME GAMES_PLAYED BEST_GAME <<< "$GAME"
